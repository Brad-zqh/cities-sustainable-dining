...
def sdi_composite():
    ...
    for metric, label, color, marker, ls in (SPEC[0], SPEC[2]):
        ...
        ax.plot(YEARS, val, color=color, lw=4.5, alpha=.12, ls=ls, zorder=1)
...
def joint_composite():
    ...
    for i, year in enumerate(ACCESS_YEARS):
        ax = fig.add_axes([xs[i]-.045,.770,.245,.208])
        cax = fig.add_axes([xs[i]+.208,.778,.011,.16])
        joint.map_panel(ax,cax,maps[year],year)
        ax.set_title(str(year),loc="center",y=1.035,pad=0,fontsize=8.5,fontweight="normal")
        ax.text(-.025,1.035,chr(97+i),transform=ax.transAxes,ha="right",va="bottom",
    ...
    fig.legend(handles=[
        ...
        loc="center",bbox_to_anchor=(.51,.750),ncol=3,fontsize=6.2,
    ...
    ax=fig.add_axes([.105,.065,.350,.132])
    ybase=np.arange(3)[::-1]
    ...
    ax=fig.add_axes([.610,.065,.275,.132])
    ...
    cmap=LinearSegmentedColormap.from_list("unsigned_blue_red",["#ADCFE4","#F4F3ED","#D98781"])
    ...
    cax=fig.add_axes([.901,.065,.012,.132])
