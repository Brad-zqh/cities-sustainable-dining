...
def points(ax, x, y, color, marker='o', label=None):
    ax.scatter(x, y, s=84, color=color, alpha=.12, marker=marker,
               lw=0, zorder=1)
    return ax.scatter(x, y, s=31, facecolor='white', marker=marker, label=label,
...
def build(paired_pages=False):
    ...
    if paired_pages: ax.set_position([.110,.570,.315,.345])
    ...
    if paired_pages: ax.set_position([.5125,.570,.360,.345])
    ...
    ax.scatter([],[],facecolor='white',edgecolor=shared.BLUE,linewidth=1.0,s=31,marker='o',label='Low quintile')
    ax.scatter([],[],facecolor='white',edgecolor=shared.RED,linewidth=1.0,s=31,marker='s',label='High quintile')
