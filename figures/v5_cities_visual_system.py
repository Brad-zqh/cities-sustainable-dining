...
def grey_osm_basemap(
    ...
    *,
    ...
    """Add a subdued CARTO/OSM context layer without changing map evidence.

    The basemap is contemporary visual context only.  Analytical geometries,
    classifications and historical network inputs remain the project data;
    attribution and retrieval date are reported in the manuscript note.
    """
    import contextily as ctx

    if crs is None:
        raise ValueError("A valid CRS is required before adding a web basemap.")
    ctx.add_basemap(
        ax,
...
def segmented_scale_bar(ax: plt.Axes, length_km: int = 10, x: float = 0.07, y: float = 0.018) -> None:
    """Compact cartographic scale carrying only the 0 and 10 km labels."""
    ...
    for i, label in enumerate((0, None, length_km)):
        xx = x0 + i * seg
        ax.plot([xx, xx], [y0, y0 + h * 1.9], color=COLORS["ink"], lw=.55, zorder=21)
...
def save_bundle(fig: plt.Figure, output_dir: Path, stem: str, preview_dpi: int = 260) -> None:
    ...
    # The manuscript uses hue only for data encodings; every textual element
    # remains black, including direct labels, legends and colourbar ticks.
